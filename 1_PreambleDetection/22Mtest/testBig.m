...
lastPos = 0;
for i = 1 : s - m + 1
    thresCurrent = 6.5 * 11 * mean(data(i : i + m - 1));
    R(i) = dot(preambleTemp, data(i : i + m - 1));
    if R(i) >= thresCurrent
        adsbR(1) = R(i);
        adsbPos(1) = i;
        lastPos = i;
...
xlabel('点数');
ylabel('Amplitude');
axis([0 length(data) 0 150]);
scrsz = get(0,'ScreenSize'); % 获取屏幕尺寸
set(gcf, 'position', [0, scrsz(4)/1.7, scrsz(3), scrsz(4)/3]);
set(gca, 'box', 'off', 'xtick', 0:1e5:length(data), 'ytick', 0:30:150, 'fontsize', 13);
...
xlabel('点数');
ylabel('Amplitude');
axis([1 length(data) 0 3000]);
scrsz = get(0,'ScreenSize'); % 获取屏幕尺寸
set(gcf, 'position', [0, scrsz(4)/1.7, scrsz(3), scrsz(4)/3]);
set(gca, 'box', 'off', 'xtick', 1:2e5:length(data), 'ytick', 0:500:3000, 'fontsize', 13);
