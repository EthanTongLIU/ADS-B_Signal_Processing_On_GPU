...
stem(1 : length(data8), data8);
hold on;
sigPos =   38150; stem(sigPos + 1 : sigPos + 2640, data8(sigPos + 1 : sigPos + 2640), 'color', 'r');
sigPos =  129005; stem(sigPos + 1 : sigPos + 2640, data8(sigPos + 1 : sigPos + 2640), 'color', 'r');
title('data8 段信号序列');
xlabel('点数');
ylabel('Amplitude');
axis([0 26e4 0 120]);
...
% Objective: 从 data8 中提取出 adsb 信号
% Demand:    1. 需要动态生成门限
%            2. 准确提取上升沿
%            3. 排除虚假报头 
...
lastPos = 0;
for i = 1 : s - m + 1
    thresCurrent = 6.5 * 11 * mean(data8(i : i + m - 1));
    R(i) = dot(preambleTemp, data8(i : i + m - 1));
    if R(i) >= thresCurrent
        adsbR(1) = R(i);
        adsbPos(1) = i;
        lastPos = i;
...
for i = 1 : length(adsbPos)
    r1 = dot(temp1, data8(adsbPos(i) : adsbPos(i) + 2640 - 1));
    r2 = dot(temp2, data8(adsbPos(i) : adsbPos(i) + 2640 - 1));
    if r1 / r2 >= 29
